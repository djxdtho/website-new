...
import { Home, User, Package, Wrench, Calendar, X, Mail } from "lucide-react"
      ...
      <AnimatePresence>
        {open && (
          <>
            ...
            {/* Social links - horizontal row below nav */}
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 10 }}
              transition={{ delay: 0.2, duration: 0.4 }}
              className="absolute bottom-4 left-0 right-0 flex items-center justify-center gap-5"
            >
              {socials.map((s, i) => (
                <motion.a
                  key={s.label}
